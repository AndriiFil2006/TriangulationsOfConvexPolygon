...
# Example usage for a regular pentagon
# Pentagon centered at origin with radius 1
pentagon_vertices = [[0,0], [0, 1], [0.5, 2], [1, 1], [1, 0]]
pentagon = Polygon(pentagon_vertices)


print("All the verticies: ", pentagon.printPolygon())

'''
# Calculate specific distance
print(f"Distance between vertex 0 and 2: {pentagon.calcDistance(0, 2):.4f}")

# Get all distances
all_dist = pentagon.getAllDistances()
print(f"\nAll pairwise distances: {len(all_dist)} pairs")

# Get edges vs diagonals
edges = pentagon.getAllEdges()
diagonals = pentagon.getAllDiagonals()

print(f"\nEdges: {len(edges)}")
for pair, dist in edges.items():
    print(f"  {pair}: {dist:.4f}")

print(f"\nDiagonals: {len(diagonals)}")
for pair, dist in diagonals.items():
    print(f"  {pair}: {dist:.4f}")
'''

triangulation, total_length = pentagon.greedy_triangulation()
results = pentagon.compare_triangulations()
#pentagon.visualize_triangulation(triangulation)
#print(f"\nTotal diagonal length: {total_length:.4f}")